...
export default function EditSubCategoryPage() {
  ...
  useEffect(() => {
    ...
    const fetchOne = async () => {
      try {
        const res = await apiClient.get<SubCategoryItem>(`/sub-category/${id}`);
        if (!active) return;
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      ...
      try {
        ...
        setCategoryList([]);
        setProductCategoryList([]);
        setCategoryListHint("โหลดรายการหมวดหมู่ไม่สำเร็จ (สามารถกรอก Category ID เองได้)");
      } finally {
  ...
  const categoryOptions = useMemo(() => {
    ...
    const dedup = new Map<string, { categoryId: string; categoryName: string }>();
  ...
  const showCategorySelect = categoryListLoading || categoryOptions.length > 0;
  const hasCurrentCategoryInOptions = categoryOptions.some(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await apiClient.patch(`/sub-category/${id}`, {
          ...
          <div>
            ...
            {showCategorySelect ? (
              ...
              >
                ...
                {!hasCurrentCategoryInOptions && categoryId.trim() ? (
                  <option value={categoryId}>ค่าเดิม: {categoryId}</option>
                ) : null}
